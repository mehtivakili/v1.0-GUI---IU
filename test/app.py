...
def open_serial():
    ...
    try:
        ...
        baudrate = int(request.form['baudrate'])
        is_binary = request.form.get('is_binary', 'false') == 'true'
        open_serial_port(socketio,port, baudrate, is_binary)
        return jsonify(status='success')
    except KeyError as e:
        return jsonify(status='error', message=f"Missing form key: {str(e)}")
    except Exception as e:
...
def open_serial_port(sss, port, baudrate, is_binary):
    ...
    serial_running = True
    serial_thread = threading.Thread(target=read_serial_data, args=(is_binary, message_queue))
    serial_thread.start()
